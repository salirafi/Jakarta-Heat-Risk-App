...
**IMPORTANT!**  This app is a personal project, so the code might not be optimized to smoother user's experience (preparation of table data for visualization is done within the pandas framework, not pre-computed in SQL). For first time loading, the web might take a few seconds.
