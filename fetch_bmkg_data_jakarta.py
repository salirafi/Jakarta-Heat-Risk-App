...
REFERENCE_FILE = Path("jakarta_reference.csv") # output from build_jakarta_preference.py, containing list of adm4 codes to fetch forecasts for
DB_PATH = Path("heat_risk.db") # SQLite database file to save forecasts into
TABLE_NAME = "heat_forecast_jakarta" # SQLite table name to save forecasts into
LOG_DIR = Path("logs") # directory to save log files, will be created if it does not exist
...
def add_fetched_at(df: pd.DataFrame) -> pd.DataFrame:
    df = df.copy()
    df["fetched_at"] = pd.Timestamp.now(tz="Asia/Jakarta").tz_localize(None) # get local Jakarta time without timezone info for comparison
    return df
            ...
            rows.append(
                {
                    ...
                    "kotkab": location_data.get("kotkab"),
                    ...
                    "humidity_pct": pd.to_numeric(item.get("hu"), errors="coerce"), # humidity in percentage
                    "weather_desc_en": item.get("weather_desc_en"), # weather description in English
...
def interpolate_one_adm4_to_grid(df_one: pd.DataFrame, target_grid: pd.DatetimeIndex) -> pd.DataFrame:
    ...
    static_cols = [
        "adm4", "desa_kelurahan", "kecamatan", "kotkab", "provinsi",
    ...
    work["weather_desc_en"] = work["weather_desc_en"].ffill().bfill()
    ...
    out["heat_index_c"] = out.apply(
        lambda row: compute_heat_index_c(row["temperature_c"], row["humidity_pct"]),
        axis=1,
    )
    ...
    return out[
        [
            ...
            "temperature_c", "humidity_pct", "heat_index_c", "risk_level",
            "weather_desc_en", "analysis_date",
        ]
    ]
